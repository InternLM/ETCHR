rjob submit --name=qarl-flux-long-prompt-16-gpu-cps-collect-only \
--gpu=8 --memory=800000 \
--cpu=80 \
...
-- bash -ex /mnt/shared-storage-user/mllm/xinglong/Pref-GRPO/scripts/full_train/flux/qarl_long_prompt_16_gpu_cps/collect_only/finetune_qarl_flux.sh