...
def _validate_image_paths(data_json_path: str, max_errors: int = 10) -> None:
    ...
    if missing:
        ...
        raise FileNotFoundError(
            f"Missing source image files (showing up to {max_errors}):\n{sample_list}"
        )
...
def load_eval_prompts(dataset, num_prompts, seed):
    ...
    for idx in indices:
        ...
        prompt_attention_mask_file = data_item["text_ids"]
        ...
        prompt_attention_mask = torch.load(
            os.path.join(dataset.text_ids_dir, prompt_attention_mask_file),
            map_location="cpu",
            weights_only=True,
        )
        ...
        samples.append((prompt_embed, prompt_attention_mask, instruction, source_image))
    return samples


def _prepare_condition_latents(vae, image_paths, height, width, device):
    if not image_paths:
        return None
    ...
    pixel_values = processor.preprocess(images, height=height, width=width)
    if pixel_values.ndim == 3:
        pixel_values = pixel_values.unsqueeze(0)
    pixel_values = pixel_values.to(device=device, dtype=vae.dtype).unsqueeze(2)
    with torch.no_grad():
    ...
    packed_height = 2 * (int(height) // (8 * 2))
    packed_width = 2 * (int(width) // (8 * 2))
    return pack_latents(
        image_latents,
...
def _decode_qwen_latents(vae, latents, h, w):
...
def run_eval_images(
    ...
    if eval_guidance_scale > 1.0 and (
        negative_prompt_embeds is None or negative_text_ids is None
    ):
        raise ValueError(
            "eval_guidance_scale > 1.0 requires negative prompt embeddings. "
            "Please run preprocess_qwen_image_edit_embedding.py first."
        )
    ...
    for idx in assigned_indices:
        prompt_embed, prompt_attention_mask, instruction, source_image = eval_prompts[idx]
        ...
        condition_latents = _prepare_condition_latents(
            vae, [source_image], args.h, args.w, device
        )
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            decoded_image = _decode_qwen_latents(vae, latents, args.h, args.w)
        ...
        decoded_image[0].save(edited_path)
        if source_image:
            try:
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:  # Add progress bar
            ...
            if condition_latents is not None:
                cond = condition_latents.to(device=z.device, dtype=z.dtype)
                latent_model_input = torch.cat([z, cond], dim=1)
            with torch.autocast("cuda", torch.bfloat16):
                ...
                if guidance_scale > 1.0:
                    if negative_prompt_embeds is None or negative_text_ids is None:
                        raise ValueError(
                            "guidance_scale > 1.0 requires negative prompt embeddings."
                        )
                    neg_embeds = negative_prompt_embeds.to(
                        device=encoder_hidden_states.device,
                        dtype=encoder_hidden_states.dtype,
                    )
                    neg_text = negative_text_ids.to(device=prompt_attention_mask.device)
                    if neg_embeds.shape[0] != encoder_hidden_states.shape[0]:
                        neg_embeds = neg_embeds.repeat(encoder_hidden_states.shape[0], 1, 1)
                    if neg_text.shape[0] != prompt_attention_mask.shape[0]:
                        neg_text = neg_text.repeat(prompt_attention_mask.shape[0], 1)
                    uncond_pred = transformer(
                        hidden_states=latent_model_input,
                        timestep=timesteps / 1000,
                        guidance=None,
                        encoder_hidden_states_mask=neg_text,
...
def grpo_one_step(
    ...
    if condition_latents is not None:
        cond = condition_latents.to(device=latents.device, dtype=latents.dtype)
    ...
    with torch.autocast("cuda", torch.bfloat16):
        ...
        if guidance_scale > 1.0:
            ...
            neg_text = negative_text_ids.to(device=prompt_attention_masks.device)
            if neg_embeds.shape[0] != encoder_hidden_states.shape[0]:
                neg_embeds = neg_embeds.repeat(encoder_hidden_states.shape[0], 1, 1)
            if neg_text.shape[0] != prompt_attention_masks.shape[0]:
                neg_text = neg_text.repeat(prompt_attention_masks.shape[0], 1)
            uncond_pred = transformer(
                hidden_states=latent_model_input,
                timestep=timesteps / 1000,
                guidance=None,
...
def sample_reference_model(
    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        batch_prompt_attention_mask = prompt_attention_masks[batch_idx]
        ...
        condition_latents = _prepare_condition_latents(
            vae, batch_source_images, h, w, device
        )
        ...
        with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
            decoded_image = _decode_qwen_latents(vae, latents, h, w)
        save_path = save_rollout_image(
            decoded_image[0],
            args.output_dir,
            f"qwen_image_edit_{rank}_{index}.png",
        )
...
def train_one_step(
    ...
    if all_condition_latents is None:
    ...
    for i,sample in list(enumerate(samples_batched_list)):
        for _ in range(train_timesteps):
            ...
            kl_loss = None
            txt_seq_len = int(sample["txt_seq_lens"].item())
            sample_txt_seq_lens = [txt_seq_len]
            sample_hidden_states = sample["encoder_hidden_states"][:, :txt_seq_len]
            sample_attention_masks = sample["prompt_attention_masks"][:, :txt_seq_len]
            sample_img_shapes = [[
                (1, args.h // 8 // 2, args.w // 8 // 2),
                (1, args.h // 8 // 2, args.w // 8 // 2),
            ]]
...
def main(args):
    ...
    if rank == 0 and not getattr(args, "skip_path_check", False):
        main_print("--> validating dataset source image paths")
    ...
    negative_text_ids = None
    need_cfg = (
        getattr(args, "train_guidance_scale", 1.0) > 1.0
        or getattr(args, "eval_guidance_scale", 1.0) > 1.0
    )
    if need_cfg:
        ...
        if negative_prompt_embeds is None or negative_text_ids is None:
            raise FileNotFoundError(
                "CFG is enabled but negative_prompt_embed.pt / negative_text_ids.pt "
                "are missing next to data_json_path. "
                "Please run preprocess_qwen_image_edit_embedding.py first."
            )
        main_print("--> loaded negative prompt embeddings for CFG")
    ...
    if rank <= 0:
        project = _reward_project_name(args, "qwenimage_edit")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (prompt_embeds, prompt_attention_masks, instruction, source_images, target_images) in enumerate(train_dataloader):
...
def build_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Skip validating source image paths before training.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Limit number of samples loaded from the training json.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="CFG guidance scale for eval sampling.",
    )
