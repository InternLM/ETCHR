...
try:
    ...
    try:
        RewardTrainer = importlib.import_module("trl").RewardTrainer
    except Exception as exc:  # pragma: no cover - import guard
        class RewardTrainer:  # type: ignore
            def __init__(self, *args, **kwargs):
                raise ModuleNotFoundError(
                    "trl is required for training; install it or ensure it's on PYTHONPATH."
                ) from exc
from fastvideo.models.hpsv3.utils.training_utils import get_peft_state_non_lora_maybe_zero_3
...
class Qwen2VLRewardModelBT(Qwen2VLForConditionalGeneration):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            embedding_layer = getattr(self.model, "embed_tokens", None)
            if embedding_layer is None:
                embedding_layer = self.get_input_embeddings()
            if embedding_layer is None:
                raise AttributeError("No input embedding layer found for Qwen2VL model.")
